...
const joinPrivateRoom = (socket, namespace) => ({ username,matricule, room, to, from, joinConfirmation }) => {
    ...
    socket.join(to, async () => {
        ...
        try {
            ...
            if (!joinConfirmation) namespace.in(room).emit('privateChat', { username,matricule, to, room, from })
        } catch (error) {
            console.log(error)
        }
    })
